...
function createReaderPublicPdfUrl(string $storagePath): string
{
    ...
    $normalizedPath =
        normalizeReaderStoragePath($storagePath);
...
function generateReaderApaCitation(
    array $authors,
    array $publication,
    string $title
): string {
    ...
    if ($authorCount === 1) {
        ...
        $authorText =
            implode(', ', $citationAuthors)
            . ', & '
            . $lastAuthor
            . ' ';
    }

    $year = !empty($publication['year'])
    ...
    $volume = !empty($publication['volume'])
        ? (string) $publication['volume']
        : '';

    $number = !empty($publication['number'])
...
if ($article) {
    ...
    if (!empty($article['journalPDF'])) {
        ...
        $pdfUrl = createReaderPublicPdfUrl(
            (string) $article['journalPDF']
        );
    }

    $citation = generateReaderApaCitation(
        $authors,
        $publication,
        (string) (
